...
YK_SECRET_KEY = os.getenv("YK_SECRET_KEY", "test_secret")
...
async def deliver_items_and_record(uid: int, items: dict[str, dict]) -> str:
    """Списываем коды, создаём заказ, возвращаем текст с кодами."""
    total = 0
    lines = []
    async with aiosqlite.connect(DB_PATH) as db:
...
async def callbacks(cb: types.CallbackQuery):
    ...
    if data.startswith("buy_now:") or data == "checkout":
        ...
        if Payment is None:
            await cb.message.answer("⚠️ Оплата временно недоступна.")
            await cb.answer(); return
        ...
        try:
            ...
            await cb.message.answer("❌ Не удалось создать платёж в ЮKassa.")
            await cb.answer(); return
    ...
    if data.startswith("paid:"):
        ...
        try:
            ...
            await cb.answer("Не удалось проверить платёж.", show_alert=True)
        return
...
# =============================
# Admin commands (управление товарами и кодами)
# =============================
...
async def admin_menu(message: types.Message):
    ...
    await message.answer(
        ...
        "• /listproducts — список\n\n"
        "Коды:\n"
        "• /uploadcodes $номинал — загрузка кодов\n"
        "• /stock — остатки\n"
        "• /export_codes — экспорт в CSV\n\n"
        "Бэкапы:\n"
        "• /backup_now — прислать бэкап\n\n"
        "Прочее:\n"
        "• /orders — последние заказы\n"
        "• /users — количество покупателей\n"
        "• /stats — статистика"
    )


# (здесь идут addproduct, setprice, delproduct, uploadcodes, stock, orders, users, stats, backup_now, export_codes
# — я могу их расписать, если нужно полностью видеть все блоки админки)
